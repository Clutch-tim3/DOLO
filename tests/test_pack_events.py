...
@pytest.fixture()
def pack() -> str:
    """A pack id no run has used before.

    These tests originally used fixed ids like "pack-order" and asserted an
    exact event count. That holds exactly once per database. `db_paths.resolve`
    seeds a fresh AGENT_DB_DIR by COPYING the repo-root agent_memory.db, so the
    rows written by the first run came back in every run afterwards and the
    counts climbed 3, 6, 9 — three failures that looked like a narration bug
    and were a test-isolation bug.

    A unique id per test is also what production does: pack ids are uuids.
    """
    return f"pack-{uuid.uuid4().hex}"


def test_events_come_back_in_order_with_increasing_seq(pack):
...
def test_polling_with_a_cursor_returns_only_what_is_new(pack):
...
def test_messages_are_rendered_from_the_detail(pack):
...
def test_a_missing_template_field_does_not_lose_the_event(pack):
...
def test_an_unknown_stage_still_records(pack):
...
def test_usage_counts_model_calls_explicitly_not_by_inference(pack):
    """
...
def test_usage_is_explicit_that_token_counts_are_not_available(pack):
    """
